...
if ($_POST['idConto']!='' and $_POST['nomecampo']!='' and $_POST['valorecampo']!='') {
    ...
    if ($stmtOK and aggiornaCategorieConti($_POST['idConto'])) {
        ...
        //ora aggiorno i figli
        $querySEL="SELECT * FROM pcs_piano_conti WHERE id=?";
        $stmtSEL = $dbh->prepare($querySEL);
        $stmtSEL->execute(array($_POST['idConto']));
        $CONTO= $stmtSEL->fetch(PDO::FETCH_ASSOC);

        $queryUPD="UPDATE pcs_piano_conti SET categoria=? WHERE id_azienda=? and livello1=?";
        $stmtUPD=$dbh->prepare($queryUPD);
        $stmtUPD->execute(array($CONTO['categoria'],$CONTO['id_azienda'],$CONTO['livello1']));
